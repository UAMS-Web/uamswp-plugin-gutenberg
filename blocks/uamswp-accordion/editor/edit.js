...
const Edit = ( props ) => {
    ...
    const addSizeClassName = ( value ) => {
        ...
        if ( value ) {
            ...
            setAttributes( { className: addBlockClassName( blockClasses, 'uams-accordion--size-', 'small' ) } );

        } else {

            setAttributes( { className: removeBlockClassName( blockClasses, 'uams-accordion--size' ) } );
		...
		<>
			<InspectorControls>
                <PanelDisplayOptions isOpen={true} >
                    ...
                    <ToggleControl
                        label="Reduce Size"
                        checked={ hasBlockClassName( ( attributes.className ?? '' ), 'uams-accordion--size-small'  ) }
                        onChange={ (value) => addSizeClassName( value ) }
                        />
                </PanelDisplayOptions>
			</InspectorControls>
            <div  { ...blockProps }>
                <h2 className="uams-accordion__title">
                    <RichText
                ...
                <div className="uams-accordion__content">
                    <div className="uams-accordion__content-inner">
                        <InnerBlocks
                            templateLock={ false }
                        />
                    </div>
