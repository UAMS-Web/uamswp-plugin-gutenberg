...
const ProgramsList = function (el) {
  ...
  const searchFilter = el.querySelector(".uams-programs-list__search-input");
  ...
  function removeEmptyGroups(programs, programsContainer) {
    ...
    const programGroups = programsContainer.querySelectorAll(
      ".uams-programs-list__list-group"
    );
  ...
  function removeDegreeTagsNotAvailableAtSelectedCampuses(programsContainer) {
    ...
    if (checkedCampusFilters.length > 0) {
      const degreeTags = programsContainer.querySelectorAll(
        ".uams-programs-list__degree-type"
      );
  ...
  function getProgramData() {
    const programEls = el.querySelectorAll(".uams-programs-list__list-item");
  ...
  function openSelectMenu(toggle) {
    uamsAriaExpanded(toggle, true);
    const icon = toggle.querySelector(".uams-programs-list__button-icon");
    icon.classList.remove("uams-i-arrow-down-carrot");
    icon.classList.add("uams-i-x-close");
  }

  function closeSelectMenu(toggle) {
    uamsAriaExpanded(toggle, false);
    const icon = toggle.querySelector(".uams-programs-list__button-icon");
    icon.classList.remove("uams-i-x-close");
    icon.classList.add("uams-i-arrow-down-carrot");
  }

  function bindToggleEvents() {
    filterToggles = el.querySelectorAll(".uams-programs-list__control-button");

    // toggling select filters
    document.addEventListener(
      "click",
      function (e) {
        const toggle = e.target.closest(".uams-programs-list__control-button");
        const insideSelectFilter =
          e.target.closest(".uams-programs-list__control--select") !== null;
...
document.querySelectorAll(".uams-programs-list").forEach((el) => {
  new ProgramsList(el);
});
