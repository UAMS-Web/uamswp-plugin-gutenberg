...
const initSwiper = () => {
  const carouselName = ".uams-carousel";
  ...
  if (carouselSliders.length > 0) {
    ...
    carouselSliders.forEach((carousel, index) => {
      ...
      carousel.classList.add(`uams-carousel-${index}`);

      const slideWrapper = document.querySelector(`.uams-carousel-${index} .uams-carousel__wrapper`);
      ...
      const swiperSettings = {
        slidesPerView: desktopCols,
          spaceBetween: 30,
          pagination: {
            el: ".uams-carousel__pagination",
            clickable: false,
          },
          navigation: {
            nextEl: '.uams-carousel__button-next',
            prevEl: '.uams-carousel__button-prev',
          },
          a11y: {
            enabled: true,
      ...
      const carouselSlider = new Swiper(`.uams-carousel-${index}`, swiperSettings);
